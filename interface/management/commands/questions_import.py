...
from interface.models import ProgrammingLanguage, ExpertiseLevel, QuizQuestion, TestCase
from interface.embeddings import store_question_embedding


class Command(BaseCommand):
    help = 'Import coding questions from a JSON file with support for multiple expertise levels'

    def add_arguments(self, parser):
        parser.add_argument('file_path', type=str, help='Path to JSON file containing questions')
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            for idx, question_data in enumerate(questions_data, 1):
                try:
                    ...
                    # Build full question text (include all parts for context)
                    ...
                    # Create test case if sample input and output are provided
                    ...
                    if not skip_embeddings:
                        try:
                            store_question_embedding(question.id, 'coding', full_question_text)
        ...
        self.stdout.write(self.style.SUCCESS(
            f'\n✅ Successfully imported {total_imported} questions. Skipped {skipped_count} questions.'
        ))
